...
export class MarcasComponent implements OnInit {
  ...
  loadMarcas() {
    this.isLoading = true;
    this.http.get<Marca[]>('http://127.0.0.1:8000/api/marcas/').subscribe({
      next: data => {
        this.marcas = data;
  ...
  saveMarca() {
    ...
    const url = this.isEditMode
      ? `http://127.0.0.1:8000/api/marcas/${this.selectedMarca.id}/`
  ...
  deleteMarca(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.delete(`http://127.0.0.1:8000/api/marcas/${id}/`).subscribe({
          next: () => {
