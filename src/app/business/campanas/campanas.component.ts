...
export class CampanasComponent implements OnInit {
  ...
  loadCampanas() {
    this.isLoading = true;
    this.http.get<Campana[]>('http://127.0.0.1:8000/api/campanas/').subscribe({
      next: data => {
        this.campanas = data;
        this.filterCampanas();
  ...
  saveCampana() {
    ...
    const url = this.isEditMode
      ? `http://127.0.0.1:8000/api/campanas/${this.selectedCampana.id}/`
      : 'http://127.0.0.1:8000/api/campanas/';
  ...
  deleteCampana(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.delete(`http://127.0.0.1:8000/api/campanas/${id}/`).subscribe({
          next: () => {
