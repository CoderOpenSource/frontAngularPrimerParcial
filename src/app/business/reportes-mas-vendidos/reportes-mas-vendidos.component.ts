...
export class ReportesMasVendidosComponent implements OnInit {
  ...
  cargarFiltros() {
    this.http.get<Categoria[]>('http://127.0.0.1:8000/api/categorias/')
      .subscribe(data => this.categorias = data);

    this.http.get<Marca[]>('http://127.0.0.1:8000/api/marcas/')
  ...
  onCategoriaChange() {
    ...
    this.http.get<Subcategoria[]>(`http://127.0.0.1:8000/api/subcategorias/?categoria=${this.categoriaId}`)
  ...
  generarReporte() {
    ...
    const endpoint = this.formato === 'pdf'
      ? 'http://127.0.0.1:8000/api/reportes/productos-mas-vendidos/pdf/'
      : 'http://127.0.0.1:8000/api/reportes/productos-mas-vendidos/excel/';
