...
export class SubcategoriasComponent implements OnInit {
  ...
  loadCategorias() {
    this.http.get<Categoria[]>('http://127.0.0.1:8000/api/categorias/').subscribe({
  ...
  loadSubcategorias() {
    this.isLoading = true;
    this.http.get<Subcategoria[]>('http://127.0.0.1:8000/api/subcategorias/').subscribe({
      next: data => {
  ...
  saveSubcategoria() {
    ...
    const url = this.isEditMode
      ? `http://127.0.0.1:8000/api/subcategorias/${this.selectedSubcategoria.id}/`
      : 'http://127.0.0.1:8000/api/subcategorias/';
  ...
  deleteSubcategoria(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.delete(`http://127.0.0.1:8000/api/subcategorias/${id}/`).subscribe({
          next: () => {
