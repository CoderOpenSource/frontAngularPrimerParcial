...
export class MovimientoInventarioComponent implements OnInit {
  ...
  loadMovimientos() {
    this.isLoading = true;
    this.http.get<MovimientoInventario[]>('http://127.0.0.1:8000/api/movimientos/').subscribe({
      next: data => {
  ...
  saveMovimiento() {
    this.isSaving = true;
    this.http.post('http://127.0.0.1:8000/api/movimientos/', this.nuevoMovimiento).subscribe({
      next: () => {
