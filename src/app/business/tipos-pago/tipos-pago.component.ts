...
export class TiposPagoComponent implements OnInit {
  ...
  loadTiposPago() {
    this.isLoading = true;
    this.http.get<TipoPago[]>('http://127.0.0.1:8000/api/tipos-pago/').subscribe({
      next: data => {
        this.tipos = data;
  ...
  saveTipo() {
    ...
    const url = this.isEditMode
      ? `http://127.0.0.1:8000/api/tipos-pago/${this.selectedTipo.id}/`
  ...
  deleteTipo(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.delete(`http://127.0.0.1:8000/api/tipos-pago/${id}/`).subscribe({
          next: () => {
