...
export class DescuentosComponent implements OnInit {
  ...
  loadDescuentos() {
    this.isLoading = true;
    this.http.get<Descuento[]>('http://127.0.0.1:8000/api/descuentos/').subscribe({
      next: data => {
  ...
  loadProductos() {
    this.http.get<any>('http://127.0.0.1:8000/api/productos/?page=1').subscribe({
      next: data => {
  ...
  loadCampanas() {
    this.http.get<CampanaDescuento[]>('http://127.0.0.1:8000/api/campanas/').subscribe(data => this.campanas = data);
  }
  ...
  saveDescuento() {
    ...
    const request = this.isEditMode
      ? this.http.put(`http://127.0.0.1:8000/api/descuentos/${this.selectedDescuento.id}/`, payload)
      : this.http.post('http://127.0.0.1:8000/api/descuentos/', payload);
